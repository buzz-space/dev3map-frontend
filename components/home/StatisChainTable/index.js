...
export default function StatisChainTable() {
  const { data, refetch, isLoading } = useGetChainList();
  ...
  return (
    <Container className={styles['container']} id="chains">
      ...
      {
        (isLoading || (dataTable?.length == 0 && value == '')) ?
          <div className='mt-8'>
            <Loading type='BarLoader' />
          </div>
          :
          (dataTable?.length > 0) ? <div className={styles['container-table']}>
            ...
            <p className='text-white text-[24px] md:text-[40px] uppercase font-bold text-center my-8'>NO DATA</p>
          </>
      }
    </Container>
  );
